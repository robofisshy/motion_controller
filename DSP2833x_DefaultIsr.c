...
float m_kp = 0.03;//0.03//0.05//0.8;//0.5//0.2//0.5;//0.005;
float m_ki =0;//0.0003;//0.0003;//0.0008//0.001;//0.005;//0.05//0.08
int adjust=0;
...
/*				Master_tx[3]=0;
		...
		if(Axis_1==1)
		{
				...
				while((Uint16)Master_rx[0]%1800!=0||(Uint16)Master_rx[15]%1029!=0)
				{
					DELAY_US(500000);
					Motor1_T();
					Motor_R();
				...
				Actual0[point]=(float)(Master_rx[8]*65536)+(float)Master_rx[7];

				e=Target1[point]-Actual0[point];
				//if(fabsf(e)<6000)
				//	e=0;						//位置式PID
					//adjust=0;
				//else
				//{
					e_sum+=e;
					ec=e-last_e;
					Master_tx[3]=floor(m_kp*e+m_ki*e_sum);		//位置式PID
					//adjust=floor(m_kp*ec+m_ki*e);
					//Master_tx[3]+=adjust;
				//}
				if(Master_tx[3]>=800)
					Master_tx[3]=800;
				else if(Master_tx[3]<=-800)
					Master_tx[3]=-800;
				act_v[point]=Master_tx[3];
				last_e=e;
		}
		if(Axis_2==1)
		{
			...
			//while((Uint16)Master_rx[0]%1800!=0||(Uint16)Master_rx[15]%1029!=0)
			while((Uint16)Master_rx[0]!=1800||(Uint16)Master_rx[15]!=1029)
			{
				//Motor_err();
				DELAY_US(500000);
				Motor2_T();
			...
			if(Master_tx[3]>=800)
				...
				Master_tx[3]=-800;
			act_v[point]=(Uint16)Master_tx[3];
			last_e=e;

		}
			PieCtrlRegs.PIEACK.bit.ACK1=1;	//中断应答寄存器写1清零，可以接受该组其他中断
			CpuTimer0Regs.TCR.bit.TIF=1;	//标志位置0，重新计时
			CpuTimer0Regs.TCR.bit.TRB=1;	//Timer0重载
			break;
		}
		case(0x22):
		{
...
interrupt void SPIRXINTA_ISR(void)    // SPI-A
{
  ...
  // Remove after inserting ISR Code
   ...
   for(;;);
//	Motor_R();
/*	while((Master_rx[0]!=0x0608&&Master_rx[0]!=0x600&&Master_rx[0]!=0x208&&\
				Master_rx[0]!=0x8)||(((Uint16)Master_rx[15]%1029)!=0&&Master_rx[15]!=0x404&&\
						Master_rx[15]!=0x401&&Master_rx[15]!=0x400&&Master_rx[15]!=0x5))
		{
			DELAY_US(500000);
			Motor_R();
...
/*
	Actual[point]=Master_rx[8]*65536+Master_rx[8];
	Actual0[point]=Actual[point];
	if((Actual[point]-Actual[point-1]>20000)&&(point!=0))
		Actual[point]=Actual[point-1]+300;
	else if((Actual[point]-Actual[point-1]<-20000)&&(point!=0))
		Actual[point]=Actual[point-1]-300;
	e=Target[point]-Actual[point];
	Master_tx[3]=floor(m_kp*e);

	if(Master_tx[3]>=400)
		Master_tx[3]=400;
	else if(Master_tx[3]<=-400)
		Master_tx[3]=-400;
...
interrupt void ECAN0INTB_ISR(void)  // eCAN-B
{
  ...
  // Remove after inserting ISR Code
	...
	ECanbShadow.CANRMP.bit.RMP25 = 1;     	         // Clear TA5
	ECanbRegs.CANRMP.all = ECanbShadow.CANRMP.all;

	vel=ECanbMboxes.MBOX25.MDL.byte.BYTE2;
	Can_cmd=ECanbMboxes.MBOX25.MDH.all;
	if(Can_cmd==0x11)
	{
		PieCtrlRegs.PIEIER1.bit.INTx7=1;				 //开定时器0中断
		point=0;
		Init_pos=ECanbMboxes.MBOX25.MDL.byte.BYTE3;
		Axis_1=ECanbMboxes.MBOX25.MDL.byte.BYTE0;
		Axis_2=ECanbMboxes.MBOX25.MDL.byte.BYTE1;
		Get_Motor_pos();								 //初始化轨迹
	}
	else if((Can_cmd==0x22)||(Can_cmd==0)||(Can_cmd==0x33))									 //停止转动
	{
		Axis_1=ECanbMboxes.MBOX25.MDL.byte.BYTE0;
		Axis_2=ECanbMboxes.MBOX25.MDL.byte.BYTE1;
